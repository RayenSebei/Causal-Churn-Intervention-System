...
This is the canonical training module. ``src.model`` remains as a thin
...
from src.preprocessing import clean_telco_data, load_telco_data
...
def load_featured_frame(csv_path: str | Path) -> pd.DataFrame:
    ...
    return add_feature_columns(clean_df)
...
def train_baseline_model(
    ...
    *,
    ...
    eval_metrics = compute_metrics(
        y_test.values, test_prob, threshold=model_config.classification_threshold
    )
    ...
    calibration_path = plot_calibration_curve(
        y_test.values, test_prob, output_path=calibration_output_path
    )
    ...
    artifacts = {
        ...
        "classification_report": classification_report(
            y_test,
            (test_prob >= model_config.classification_threshold).astype(int),
            zero_division=0,
        ),
        "y_test": y_test,
        "y_prob": test_prob,
...
def train(
    ...
    *,
    ...
    """Public training entry point used by scripts and callers."""
...
def run_cli() -> None:
    ...
    metrics, artifacts = train_baseline_model(
        ...
        calibration_output_path=paths.calibration_plot,
    )
    print(metrics)
